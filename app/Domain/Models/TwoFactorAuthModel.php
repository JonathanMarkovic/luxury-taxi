...
class TwoFactorAuthModel extends BaseModel
{
    ...
    public function findByUserId(int $userId): ?array
    {
        ...
        $sql = "SELECT * FROM two_factor_auth WHERE user_id = :user_id";
    ...
    public function create(int $userId, string $secret): int
    {
        ...
        if (!is_array($user)) {
            ...
            $sql = "INSERT INTO two_factor_auth (user_id, secret, enabled) VALUES (:user_id, :secret, :enabled)";
    ...
    public function enable(int $userId): bool
    {
        ...
        $sql = "UPDATE two_factor_auth SET enabled = 1 WHERE user_id = :user_id";
    ...
    public function isEnabled(int $userId): bool
    {
        ...
        // $sql = "SELECT * from two_factor_auth WHERE user_id = :user_id AND enabled = 1";
        $sql = "SELECT * from two_factor_auth WHERE user_id = :user_id ";
        $result = $this->selectOne($sql, [
            'user_id' => $userId
        ]);
        ...
        // if ((int)$result["enabled"] !== null) {
        //     return true;
        // } else {
        //     return false; // Replace with your implementation
        // }
        // dd($result);
        return (int)$result["enabled"] !== 0;
    }
